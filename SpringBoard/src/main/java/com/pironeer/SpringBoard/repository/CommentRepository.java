...
public class CommentRepository {
    ...
    public void save(Comment comment) {
        if (comment.getCmtId() == null) { // 새 댓글인 경우
            Long id = commentIdxGenerator.incrementAndGet(); // id를 생성
            comment.setCmtId(id); // comment 객체의 cmt_id에 생성된 id 넣기
            commentMap.put(id, comment); // 맵에 생성된 id와 댓글 객체를 넣기
        } else { // 이미 해당 id값이 있는 경우
            commentMap.replace(comment.getCmtId(), comment); // 새로운 댓글로 교체
        }
    }
    ...
    public List<Comment> findByTopicId(Long topicId) { // 댓글을 조회하고 싶은 게시글 id를 인자로 전달
        return commentMap.values().stream() // 저장된 모든 댓글을 스트림으로 변환
                .filter(comment -> comment.getTopicId().equals(topicId)) // 전달받은 인자 값과 댓글의 값이 같은지 비교
    ...
    public List<Comment> findByParentId(Long parentId) { // 모든 대댓글을 조회하고 싶은 부모 댓글 id를 인자로 전달
        return commentMap.values().stream() // 저장된 모든 댓글을 스트림으로 변환
                .filter(comment -> parentId.equals(comment.getParentId())) // 해당 댓글들 필터링
                .collect(Collectors.toList());
    }

    //id로 댓글 찾기
    public Optional<Comment> findById(Long cmtId) {
        return Optional.ofNullable(commentMap.get(cmtId));
    }

    public void deleteCmtById(Long cmtId) {
        commentMap.remove(cmtId);
    }
}
